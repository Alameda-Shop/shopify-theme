...
function buildLabels(data) {
	...
	if (bcSfFilterConfig.custom.show_discount && onSale) {
		...
		if (bcSfFilterConfig.custom.discount_mode === 'saving') {
			saveAmount = bcsffilter.formatMoney(data.compare_at_price_min - data.price_min, null, true);
		} else {
...
function buildPrice(data) {
	...
	if (data.price_min) {
		...
		var priceAmount = priceAmount = bcsffilter.formatMoney(data.price_min, null, true);
		if (priceVaries) {
			priceAmount = bcSfFilterConfig.label.from.replace(/{{price_min}}/g, bcsffilter.formatMoney(data.price_min, null, true));
		}
		priceHtml = priceHtml.replace(/{{amount}}/g, priceAmount);
		priceHtml = priceHtml.replace(/{{priceClass}}/g, onSale ? 'price--highlight' : '');
      
		if (onSale) {
			priceHtml += '<span class="price price--compare" data-money-convertible="">' + bcsffilter.formatMoney(data.compare_at_price_min, null, true) + '</span>';
		}
	}
	priceHtml += '</div>';
	return priceHtml;
...
BCSfFilter.prototype.buildAdditionalElements = function (data, eventType) {
	...
	jQ('body').find('.bc-sf-filter-skeleton-button').remove();
      ...
      console.log("entro a yotpo 1")
      var api = new Yotpo.API(yotpo);
      api.refreshWidgets();
    }else {
      console.log("entro a yotpo 2")
    }
