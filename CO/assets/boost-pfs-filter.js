...
(function () { // Add this
	...
	ProductGridItem.prototype.compileTemplate = function (data, index) {
		...
		var itemInfoHtml = boostPFSConfig.custom.product_price_position == 'after_title' ?            
			'<div style="display:inline-block" class="grid-view-item__reviews"><div class="yotpo bottomLine" data-appkey="kRFK8vGgWeEOmADY36qCaTAyIruU5xpx3nre215n" data-domain="{{itemDomain}}" data-product-id="{{itemId}}" data-product-models="{{itemId}}" data-name="{{itemTitle}}" data-url="{{itemUrl}}" data-image-url="{{itemThumbUrl}}" data-description="{{itemDescription}}" data-bread-crumbs="{{itemTags}}"> </div></div>{{itemTitleWrapper}}{{itemSwatch}}{{itemPrice}}' :
		...
		// Add main attribute (Always put at the end of this function)
      	// Build Domain
        itemHtml = itemHtml.replace(/{{itemDomain}}/g, Utils.escape(boostPFSConfig.shop.domain));
